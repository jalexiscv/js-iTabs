iRotater = new Class({
  ...
  createFx: function () {
    if (!this.slideFx)
      this.slideFx = new Fx.Elements(this.slides, {duration: this.options.transitionDuration, link: 'cancel'});
...
//    });
  ...
  autoplay: function () {
    if (this.slideshowInt)
      return this;
    if (this.options.hover && !this._hoverSet)
      this.setupHover();
  ...
  stop: function (not_pause) {
    ...
    if (not_pause && this.options.hover)
      this.removeHover();
    return this;
  },
  rotate: function () {
